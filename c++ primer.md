...
#### 定义一个类型成员、内联成员函数、重载成员函数

```c++
class Screen {
public:
    typedf string::size_type pos; //自定义类型成员pos
    Screen() = default; // 默认构造函数
    Screen(pos ht, pos wd, char c) : height(ht), width(wd) {}
    char get() const {return cursor;}  //隐式内联 inline
    inline char get(pos ht, pos wd) const;  //显式内联 inline 重载get函数
